...
public class EvaluateExistingDocuments
{
    ...
    private readonly ILogger<EvaluateExistingDocuments> _logger;
    ...
    public EvaluateExistingDocuments(IAuthorizationValidator authorizationValidator, IJsonConvertWrapper jsonConvertWrapper, ILogger<EvaluateExistingDocuments> logger, 
        IValidatorWrapper<EvaluateExistingDocumentsRequest> validatorWrapper, IDocumentEvaluationService documentEvaluationService, IExceptionHandler exceptionHandler)
    {
    ...
    public async Task<HttpResponseMessage> Run([HttpTrigger(AuthorizationLevel.Function, "post", Route = "evaluateExistingDocuments")] HttpRequestMessage request)
    {
        ...
        const string loggingName = "EvaluateExistingDocuments - Run";
        ...
        try
        {
            ...
            var authValidation =
                await _authorizationValidator.ValidateTokenAsync(request.Headers.Authorization, currentCorrelationId, PipelineScopes.EvaluateExistingDocuments, 
                    PipelineRoles.EvaluateExistingDocuments);
            ...
            var evaluateExistingDocumentsRequest = _jsonConvertWrapper.DeserializeObject<EvaluateExistingDocumentsRequest>(content);

            var results = _validatorWrapper.Validate(evaluateExistingDocumentsRequest);
            if (results.Any())
